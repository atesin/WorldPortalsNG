...
- Minor syntax fixes
- deprecated "int countLines(String text)" method
- new method "int setText(String multilineText)" that split text in lines, set them as property, and returns number of pages
- removed print() method "text" parameter as now it prints text lines property
...
- Set text, optionally you can get how many pages will have your text before print
- Print some of your text's page

- Create a TextTab object, specifying default format code, page height and tab positions (line = 53chars)
- Set text and return number of pages
- Print some page
- You can set another text and print some page indefinitely with this same object

<i>
TablePrinter myPrinter = new TablePrinter("\u00A7E",(int) height, -10, 15); // can omit last left aligned width 
String mlText = "HEAD1\tHEAD2\tHEAD3\n" + "\u00A7Cdata1\tdata2\tdata3`show`/list";
int numPages = setText(mlText);                                // read number of pages optional
myPrinter.print((CommandSender) myReader, String otherText);   // quick print all pages of ahother text
myPrinter.print((CommandSender) myReader, String anotherText); // quick print all pages of ahother text
myPrinter.print((CommandSender) myReader, (int) myPage);       // print specificed page, if 0 will print all,
</i>

PRINT TEXT LINKS:

As a bonus, this class provides a static method to print text links to do more friendly interfaces
Delimit link attributes with grave accents ("printed text`tooltip`command", not to confuse with acute accents)
...
public class TablePrinter
{
	...
	private List<Integer> hTabs = new ArrayList<Integer>();
	...
	// set text and get pages according previously specified height
	int setText(String multilineText)
	{
		textLines = multilineText.split("\r?\n");
		numPages = (int) Math.ceil( ((double)textLines.length) / pageHeight);
		return numPages;
	}
	
	
	// quick print all pages from other text
	void print(CommandSender sender, String multilineText)
	{
		print(sender, multilineText.split("\r?\n"), 0);
	}
	
	
	// print specified page from setted text
	void print(CommandSender sender, int page)
	{
		print(sender, textLines, page);
	}
	
	
	// here the magic happens
	private void print(CommandSender sender, String[] textLines, int page)
	{
		// previous tasks
		...
		// define lines range
		int from = 0;
		int to = textLines.length;
		if (page > 0)
		{
			from = (Math.min(page, numPages) - 1) * pageHeight;
			to = Math.min(from + pageHeight, textLines.length);
		}
		...
		for (int l = from; l < to; ++l)
		{
			...
			line = "";
			String[] fields = textLines[l].split("\t");
			for (int f = 0; f < fields.length; ++f)
			{
				...
				if (f < hTabs.size())
					line += formatField(field, tab, monospace, true);
				else
				{
					line += formatField(field, tab, monospace, tab < 0);
					break;
				}
			}
			if (monospace)
	...
	private String formatField(String field, int tab, boolean monospace, boolean fill)
	{
		...
		while (pxLen(text, monospace) > tab)
		{
		...
		// build fill space
		String blank = fill ? fillSpaces(tab - pxLen(text, monospace), monospace, leftHand) : "";
	...
	private String fillSpaces(int len, boolean monospace, boolean leftHand)
	{
		String blank = "";
	...
	private int pxLen(String field, boolean monospace)
	{
	...
	/**
	 * prints text links on server console or client chat area
	 * @param sender the message destination
	 * @param raw the actual message with tab delimited fields: "text`tooltip", "text`tooltip`command" or "text`tooltip`suggest`"
	 */
